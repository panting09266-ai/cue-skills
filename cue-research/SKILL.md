...
**被动触发（①②③）只给一行文字 + 链接，不展示大二维码；二维码 `../assets/community-group-qr.png` 仅在 ④（用户主动要）时展示。** 冷却 `~/.cue/last-community-invite.json`（被动每会话最多一次、距上次 <14 天跳过；读写失败则本会话不再弹）。**当前为内部群，外部租户用户无法加入**——完整规则与边界见 [`../community-invite.md`](../community-invite.md)。
