...
name: cue-buddy
version: 0.1.0
description: "Cue 搭子模板作者工具：在任何 AI agent (Claude Code / Codex / Gemini / OpenClaw) 里通过 Cue 生产 API 起草、校验、创建、调试自己的搭子模板，并提交到个人模板库。用户场景包括：基于业务场景定义一个新的搭子（如对公授信预尽调、KYC 反洗钱、财报点评、市场异动等）、调试已有搭子在真实任务上的表现、把调好的模板发布到自己的工作台。触发短语：创建搭子 / 做一个 X 搭子 / 调试模板 / 测试我的搭子 / 提交模板 / 发布搭子。"
metadata:
  requires:
    bins: ["python3"]
...
**一个"搭子"是 Cue 里的专属调研伙伴**：你预先定义场景（对公授信预尽调 / KYC 反洗钱筛查 / 季度财报点评 / 财富投顾对比 / 私募尽调 ...），把"满意的研究路径"沉淀成模板；之后只需输入主体名/重点，搭子就按你定好的**调研策略**自动取证、按你定好的**报告骨架**自动产出结构化报告。Cue 的产品命题是"**把满意经历沉淀成你身边的 AI 伙伴**"——搭子模板就是这个沉淀的载体。
...
| `goal` | 你是谁、给谁服务、解决什么痛点 | 单段攻击力句式，不写编号清单 |
...
- **目标用户**：懂业务场景（金融、咨询、合规、研究、医疗等专业领域）的非技术人员
...
|---|---|---|---|
| `+author` | 引导式起草新模板：问几个业务问题 → agent LLM 起草 4 字段 → 自动跑 `+validate` | 否 | (本地) |
...
| `+publish <template_id>` | 把模板钉到 cuecue.cn 工作台首页"常用"区 | 否 | `POST /api/templates/frequent` |
| `+unpublish <template_id>` | 从首页"常用"区取消钉选 | 否 | `POST /api/templates/frequent` (`is_frequent=false`) |

**P1+P2 已发布**：`+author / +validate / +create / +list / +get / +update / +test / +tune / +publish / +unpublish`。

**关于 `+publish`**：Cue 当前没有"跨用户发布"原语；`+publish` 实际上是把模板钉到调用者自己工作台首页的"常用"区（`is_frequent=true`），方便高频访问。"对外分享"能力须走 cuecue.cn 网页端的分享/复制链路。
...
"上线 tpl_xxx" / "钉到首页" / "publish" / "pin to home"           → +publish
"从首页摘掉 tpl_xxx" / "unpin" / "下架"                           → +unpublish
──────────────────────────────────────────────────────────────────────────
```
...
> "你手头有没有这个场景下的参考资料？例如：
> - 已有的报告样例（PDF / Word / Markdown / 粘贴文本）
> - 行业 SOP / 内部规范 / 监管文件
> - 同类竞品搭子的描述
> - 相关链接（公司官网 / 行业研报 / 监管页面）
>
> 把文件路径或链接给我，或者直接把内容粘进来。"
...
1. **场景与角色**
   - 你的目标用户是谁？（例如：银行授信岗、券商投研、医院科主任）
   - 这个搭子要解决用户什么具体痛点？
   - 是参考已有的某个示例（如 `references/examples/corporate-credit.md`），还是从零起？
   → 起草 `goal` + `title` + 类别
...
2. **`goal` 必须单段攻击力句式** — 角色+痛点+价值，不写编号清单
...
- 后续将补充：二级投研、KYC 反洗钱、财报点评、市场异动快讯 等
...
- `+publish` 写操作前必须用户口头确认意图
- 任何 `+test / +tune` 调用前向用户提示"这次约消耗 N credits"
...
| Codex CLI | ⚠️ 未独立验证 | 手动 `cat SKILL.md` 注入或按 codex skill 约定加载 |
| Gemini CLI | ⚠️ 未独立验证 | `activate_skill` 加载 SKILL.md |
...
本 skill **额外使用**了若干内部 endpoint：`POST /api/templates`（create）、`PUT /api/templates/<id>`（update）、`GET /api/templates/<id>`（get one）、`POST /api/generate_template`（+tune 后端）、`POST /api/templates/frequent`（+publish）。这些 endpoint 与前端工作台共享同一套 auth 中间件，API key 可调用，但**未列在公开 API 文档中**——意味着未来可能调整 path/payload。

如果你在 +create / +update / +tune / +publish 上遇到 4xx 错误，先检查 [Cue 官方文档](https://sensedeal.feishu.cn/wiki/NS0ywPa4jiN4dgkA8V7cQvpxndf) 是否已更新对应规范。
...
| `+publish <id>` | `cue_api.py` | `python3 scripts/cue_api.py publish <id>` |
| `+unpublish <id>` | `cue_api.py` | `python3 scripts/cue_api.py unpublish <id>` |
